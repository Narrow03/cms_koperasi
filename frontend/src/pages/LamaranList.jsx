          ...
          {lamaran.map(item => (
            <tr key={item.id}>
              ...
              <td>
                ...
                <a href={`${BACKEND_URL}/api/files/resumes/${item.namaFileResume}`} target="_blank" rel="noopener noreferrer">
                  {item.namaFileResume}
                </a>
              </td>
              <td>
                {/* Tombol untuk menghapus lamaran */}
                <button onClick={() => handleDelete(item.id)} className="cms-button delete">
                  Hapus
                </button>
              </td>
            </tr>
